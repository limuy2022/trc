 ...
 */
export template <typename floattype> bool isequal(floattype a, floattype b) {
 ...
 * @param data 数据
 */
export template <typename T> inline bool inrange(T start, T end, T data) {
 ...
 * @warning 必须为静态数组
 */
export template <typename T> constexpr size_t sizeof_static_array(T& arr) {
 ...
 */
export template <typename T> unsigned int len(T in) {
    // 负数就提升为正数
    if (in < 0) {
        in = -in;
    }
