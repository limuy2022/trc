 ...
 * @param path 路径
 */
export void loader_ctree(TVM_space::TVM* vm, const std::string& path) {
 ...
 * @param path 路径
 */
export void save_ctree(TVM_space::TVM* vm, const std::string& path) {
 ...
 * @warning 此时文件必须未被打开
 */
export bool is_magic(const std::string& path) {
    FILE* file = fopen(path.c_str(), "rb");
    if (!file)
        error::send_error(
            error::OpenFileError, language::error::openfileerror, path.c_str());
