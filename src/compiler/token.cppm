...
private:
    ...
    void setbuf(std::array<char, buffersize + 1>& buf) {
        char_ptr = buf.data();
        auto readsz = fread(buf.data(), buffersize,
            sizeof(std::remove_reference_t<decltype(buf)>::value_type), file);
        buf[readsz] = 0;
    }

public:
    // char buffer,prevent compiler from reading a large file and storing it in
    // memory
...
token token_lex::lex_int_float() {
    ...
    case token_ticks::FLOAT_VALUE: {
        if (res_len > FLOAT_LONGFLOAT_LINE) {
            ...
            result.data
                = compiler_data.const_float.add(strtod(str.c_str(), nullptr));
        }
        break;
    }
...
token token_lex::lex_english() {
    ...
    // 啥关键字都不是，只能是名称了
    return token { token_ticks::NAME, compiler_data.const_name.add(tmp) };
}
