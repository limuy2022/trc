...
static const int OBJS_POOL_SIZE = 100;
// 对象池每次扩容个数
static const int OBJS_ADD_SIZE = 15;
...
template<typename T>
void objs_pool<T>::add() {
    /**
     * 扩大容量，主要用处是减小代码膨胀并使层次更清晰
     * 原理是生成一块新的内存空间
     */
    T* add_ptr = new T[OBJS_ADD_SIZE];
    blocks.push_back(add_ptr);
    typename list<T*>::iterator add_iter = free_head.end();
...
template<typename T>
template<typename ... P>
T *objs_pool<T>::trcmalloc(P &&... data_argv) {
    if (!free_head.empty()) {
        // 内存充足，可以直接使用
...
template<typename T>
objs_pool<T>::~objs_pool() {
    typename list<T*>::iterator tmp = blocks.begin();
