...
namespace memclass {
    ...
    freespace::freespace() :
        ...
        char *node_address = membase;
        node_mem * now = head;
        for (int i = 0; i < end_condit; ++i, node_address += size_block_now) {
            now->next = new node_mem(size_block_now);
            now->next->address = node_address;
        }
...
void *memory_pool::mem_malloc(size_t size_) {
    ...
    if (fl_head->next == nullptr) {
        ...
        if (fl_head->next == nullptr) {
            // 仍然为空，向操作系统申请内存
            fl_head->next = new node_mem(fl_head->length);
            return fl_head->next->address;
        }
    }
...
memory_pool::memory_pool() {
    size_block_now = 8;
    // 分配内存链表头
    memory_heads = new freespace[MEMORY_LISTS_NUM];
    size_block_now = 8;
}
