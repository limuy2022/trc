...
TEST(token, string_lex) {
    test_tokens(R"("r\n\n\npopop")",
        { { token_ticks::STRING_VALUE, "r\n\n\npopop" } });
    test_tokens(R"("o\n\t\n\t\tup")",
        { { token_ticks::STRING_VALUE, "o\n\t\n\t\tup" } });
    test_tokens(R"('789\n712\t')",
        { { token_ticks::STRING_VALUE, "789\n712\t" } });
    test_tokens(
        R"('\'')", { { token_ticks::STRING_VALUE, "'" } });
    test_tokens(
        R"("\"")", { { token_ticks::STRING_VALUE, "\"" } });
}
...
TEST(token, float_int_lex) {
    test_tokens("1.90+3.98-1",
        { { token_ticks::FLOAT_VALUE, "1.90" },
            { token_ticks::ADD, "" },
            { token_ticks::FLOAT_VALUE, "3.98" },
            { token_ticks::SUB, "" },
            { token_ticks::INT_VALUE, "1" } });
    test_tokens("1.90001",
        { { token_ticks::FLOAT_VALUE, "1.90001" } });
}
...
TEST(token, english) {
    ...
    test_tokens("aaa a1 a_1_3_bc __a_c _a1c2_o r",
        { { token_ticks::NAME, "aaa" },
            { token_ticks::NAME, "a1" },
            { token_ticks::NAME, "a_1_3_bc" },
            { token_ticks::NAME, "__a_c" },
            { token_ticks::NAME, "_a1c2_o" },
            { token_ticks::NAME, "r" } });
    // 保留字
