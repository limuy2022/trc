...
TEST(autodll, dllfunc) {
    char* name
        = autodll::redirect_to_platform(redefine_path("autodll/create_dll"));
    autodll::dll_t test_ = autodll::dllopen(name);
    delete[] name;
    auto add_ = (add_func)autodll::dllfuncload(test_, "add");
    EXPECT_EQ(3, add_(1, 2));
    EXPECT_EQ(9, add_(2, 7));
    auto return_ = (return_func)autodll::dllfuncload(test_, "return_");
