...
static void check_bytes(
    const std::vector<def::byte_t>& expect_data,
...
static void test_bytes_order_change(
    std::vector<def::byte_t> origin_data,
    const std::vector<def::byte_t>& expect) {
    utils::bytes_order_change(
        &origin_data[0], origin_data.size());
