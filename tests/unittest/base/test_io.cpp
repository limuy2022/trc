...
TEST(io, fast_int_read) {
    ...
    bool isright = true;
    FILE* file
        = fopen_with_check(redefine_path("io/fastread_empty.in").c_str(), "r");
    ...
    std::vector<int> expect_res = { 789, 456, 123, 0, 0, 14, -7, -2369 };
...
TEST(io, fast_int_write) {
    ...
    FILE* file
        = fopen_with_check(redefine_path("io/fastwrite.out").c_str(), "w");
